...
export function ColorsDropdowInput({ error, className, onChange, value }: ColorsDropdownInputProps) {
    const [selectedColor, setSelectedColor] = useState<null | Color>(() => {
        if(!value) {
            return null;
        }

        const color = colors.find(item => item.color === value) ?? null;
    ...
    function handleSelectColor(color: Color) {
        setSelectedColor((prevState) => (
            prevState?.color === color.color ? null : {
                color: color.color,
                bg: color.bg
            }
            ))
        ...
        <div >
            <DropdownMenu.Root>
                <DropdownMenu.Trigger>
                    <button 
                        className={cn(
                            "bg-white rounded-lg border border-gray-500 px-3 h-[52px] w-full text-gray-7 00 focus:border-gray-800 transition-all outline-none text-left relative",
                    ...
                    >
                        ...
                        {!selectedColor?.color ? (
                            ...
                            <div className='w-6 h-6 text-gray-800 absolute right-5 top-1/3 -translate-y-1/3'>
